...
export function SpatialCommandCenter({ previewOwner = false }: { previewOwner?: boolean }) {
  ...
  const [geoMode, setGeoMode] = useState<"cinematic" | "satellite" | "live">("cinematic");
    ...
    <section className={`wise-brain wb-visual-${visual} wb-eye-${eyeStyle} ${drawerOpen ? "drawer-open" : "drawer-closed"}`} aria-label={isOwner ? "Friday Brain" : "Wise Brain"}>
      ...
      <div className="wise-brain-body">
        ...
        <main className="wb-stage">
          ...
          <div className="wb-geo-mode" role="group" aria-label="Visualização geográfica"><button className={geoMode === "cinematic" ? "active" : ""} onClick={() => setGeoMode("cinematic")}>Cinemático</button><button className={geoMode === "satellite" ? "active" : ""} onClick={() => setGeoMode("satellite")}>Satélite</button><button className={geoMode === "live" ? "active" : ""} onClick={() => setGeoMode("live")}>Mapa vivo</button></div>
          {geoMode !== "live" ? <>
            <div className={`wb-cinematic-globe ${geoMode === "cinematic" ? "wb-holographic" : "wb-satellite"} ${activeCountry ? "focused" : ""}`} style={focusStyle}><MapZone countries={COUNTRIES} selected={selectedCountry} onSelect={selectCountry} onViewportChange={setSharedViewport} markerVariant={geoMode === "cinematic" ? "holographic" : "satellite"} immersive projection="globe" stylePreset="satellite" hideChrome/><span className="wb-cinematic-clouds" aria-hidden/><span className="wb-holo-scan" aria-hidden/><span className="wb-globe telemetry" aria-hidden><b>ORBITAL</b><small>LIVE · 3D</small></span><i/><i/></div>
            <div className={`wb-cinematic-map ${geoMode === "cinematic" ? "wb-holographic" : "wb-satellite"} ${activeCountry ? "focused" : ""}`} style={focusStyle}><MapZone countries={COUNTRIES} selected={selectedCountry} onSelect={selectCountry} viewport={sharedViewport} markerVariant={geoMode === "cinematic" ? "holographic" : "satellite"} immersive projection="mercator" stylePreset={geoMode === "cinematic" ? "satellite" : "google"} hideChrome/><div className="wb-cine-links" aria-hidden>{Array.from({length:7}).map((_,i)=><i key={i}/>)}</div><span className="wb-holo-depth" aria-hidden/><div className="wb-map-legend" aria-label="Legenda dos países"><span className="crit">CRÍTICO</span><span className="warn">ATENÇÃO</span><span className="stable">ESTÁVEL</span></div></div>
          </> : <>
            <div className="wb-live-globe"><MapZone countries={COUNTRIES} selected={selectedCountry} onSelect={selectCountry} immersive projection="globe" /></div>
            <div className="wb-live-map"><MapZone countries={COUNTRIES} selected={selectedCountry} onSelect={selectCountry} immersive projection="mercator" /></div>
          </>}
