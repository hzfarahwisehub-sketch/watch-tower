    ...
    <section ref={rootRef} className={`wise-brain wb-visual-${visual} wb-eye-${eyeStyle} ${drawerOpen ? "drawer-open" : "drawer-closed"}`} aria-label={isOwner ? "Friday Brain" : "Wise Brain"}>
      ...
      <div className="wise-brain-body">
        ...
        <main className="wb-stage">
          ...
          {geoMode !== "live" ? <>
            <div className={`wb-cinematic-globe ${geo.cls} ${activeCountry ? "focused" : ""}`} style={focusStyle}><MapZone countries={COUNTRIES} selected={selectedCountry} onSelect={selectCountry} markerVariant={geo.marker} immersive projection="globe" stylePreset={geo.globe} showArcs={geo.arcs} spinPausesOnSelect hideChrome/><span className="wb-holo-scan" aria-hidden/><span className="wb-globe telemetry" aria-hidden><b>ORBITAL</b><small>LIVE · 3D</small></span><i/><i/></div>
